...
void EfineDist() {
    ...
    TH2D *chn_energy_dist_T0T1 = new TH2D("chns Efine data distribution (T0T1)",  Form("Efine distribution with chn correlation Vmesh=500 (T0 T1);strip;Efine[digits]"), 124, -0.5, 123.5, 120, 0, 1024);
    TH2D *chn_energy_dist_T2T3 = new TH2D("chns Efine data distribution (T2T3)",  Form("Efine distribution with chn correlation Vmesh=500 (T2 T3);strip;Efine[digits]"), 124, -0.5, 123.5, 120, 0, 1024);

    TH1D *efine_dist_T0T1 = new TH1D("Efine data distribution (T0T1)",  Form("Efine distribution Vmesh=500 (T0 T1);Efine[digits]"), 120, 0, 1024);
    TH1D *efine_dist_T2T3 = new TH1D("Efine data distribution (T2T3)",  Form("Efine distribution Vmesh=500 (T2 T3);Efine[digits]"), 120, 0, 1024);

    TH1D *saturated_efine_dist_T0T1 = new TH1D("saturated Efine distribution (T0T1)",  Form("Efine distribution (T0 T1);strip"), 124, -0.5, 123.5);
    TH1D *saturated_efine_dist_T2T3 = new TH1D("saturated Efine distribution (T2T3)",  Form("Efine distribution (T2 T3);strip"), 124, -0.5, 123.5);
    //validity check
   if (inFile.fail()) {
...
// ------------------------ TIGER 0 ------------------------------ //
        if (tigerId == 0 && mode == "EW:"){
			...
			//filling histos
                ...
                //if(timeVal > 5 && timeVal < 5.001){
				chn_time_dist_T0T1->Fill(strNum,timeVal);
                // debug:
                cout << "Tiger"<< tigerId << " : " <<"strip: "<< strNum << ", efine: " << efine << endl;
                chn_energy_dist_T0T1->Fill(strNum,efine);
                efine_dist_T0T1->Fill(efine);
                //}
...
// ------------------------ TIGER 1 ------------------------------ //
        if (tigerId == 1 && mode == "EW:") {
            ...
            switch (ch){
                ...
                efine_dist_T0T1->Fill(efine);
                if(efine >= 1024){
                    cout << "Tiger"<< tigerId << " : " <<"strip: "<< strNum << ", efine: " << efine << " SATURATION" << endl;
                    efinesat++;
                } 
                //} 
...
// ------------------------ TIGER 2 ------------------------------ //
        if (tigerId == 2 && mode == "EW:"){
            ...
            switch (ch){
                ...
                //debug
                cout << "Tiger"<< tigerId << " : " <<"strip: "<< strNum << ", efine: " << efine << endl;
                chn_energy_dist_T2T3->Fill(strNum,efine);
                efine_dist_T2T3->Fill(efine);
                //}
...
//E fine data time distribution for each tiger
	...
	TCanvas *cEfineDist_FEBT2T3 = new TCanvas("cEfineDist_FEBT2T3","cEfineDist_FEBT2T3");
    cEfineDist_FEBT2T3->cd();
	chn_energy_dist_T0T1->Draw("colz");
    //cEfineDist_FEB->cd(3);
	//chn_energy_time_dist_T4T5->Draw("colz");
    //cEfineDist_FEB->cd(4);
	//chn_energy_time_dist_T6T7->Draw("colz");

    TCanvas *cEfineDist = new TCanvas("cEfineDist","cEfineDist");
    cEfineDist->cd();
	efine_dist_T0T1->Draw();
}